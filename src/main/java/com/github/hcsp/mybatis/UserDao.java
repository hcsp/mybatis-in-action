...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            paramsMap.put("offset", (pageNum - 1) * pageSize);
            paramsMap.put("limit", pageSize);
            List<User> userList = session.selectList("UserMapper.getUserByUserName", paramsMap);
            int count = (Integer) session.selectOne("UserMapper.getPageCount", username);
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            ...
            session.insert("UserMapper.batchInsertUsers", param);
        }
    }
