...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            int num = session.selectOne("MyMapper.page", username);
            int page = (num % pageSize == 0) ? (num / pageSize) : (num / pageSize + 1);
            return Pagination.pageOf(users, pageSize, pageNum, page);
        }
    }
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            ...
            parm.put("wangba", users);
            session.insert("MyMapper.insertInto", parm);
        }
    }
