...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            parameters.put("offset", (pageNum - 1) * pageSize);
            parameters.put("limit", pageSize);
            List<User> list = session.selectList("UserMapper.getUserByPage", parameters);
            Integer total = session.selectOne("UserMapper.countUser", username);

            return Pagination.pageOf(list,
                    pageSize,
                    pageNum,
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            ...
            session.insert("UserMapper.batchInsertUsers", parameters);
        }
    }
