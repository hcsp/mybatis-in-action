...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            int userName = session.selectOne("MyMapper.countUser", username);

            int totalPage = (userName % pageSize == 0) ? userName / pageSize : userName / pageSize + 1;
    ...
    public User selectUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            User user = session.selectOne("MyMapper.getUserById", id);
            return user;
        }
    }
}
