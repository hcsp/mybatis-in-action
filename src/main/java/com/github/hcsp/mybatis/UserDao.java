...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        ...
        try (SqlSession sqlSession = sqlSessionFactory.openSession(true)) {
            List<User> userList = sqlSession.selectList("UserMapper.selectUserByPage", map);
            int count = sqlSession.selectOne("UserMapper.countUser", username);
            int totalPageSize = (count % pageSize == 0) ? count / pageSize : count / pageSize + 1;
            return Pagination.pageOf(userList, pageSize, pageNum, totalPageSize);
        }
    }
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            Map<String, Object> map = new HashMap<>();
            map.put("users", users);
            session.insert("UserMapper.insertList", map);
        }
    }
    ...
    public void updateUser(User user) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.update("UserMapper.updateById", user);
        }
    }
    ...
    public void deleteUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.delete("UserMapper.deleteUserById", id);
        }
    }
