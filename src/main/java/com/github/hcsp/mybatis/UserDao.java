...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<User> users = session.selectList("UserMapper.getUserByPage", map);
            int count = session.selectOne("UserMapper.getAllUser", username);
            int totalPage = count % pageSize == 0 ? count / pageSize : users.size() / pageSize + 1;
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            Map<String, Object> map = new HashMap<>();
            map.put("users", users);
            session.insert("UserMapper.batchInsertUser", map);
        }
    }
