...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            List<User> users = sqlSession.selectList("userMapper.selectUserByName", map);
            int count = sqlSession.selectOne("userMapper.countUserNum", map);
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession sqlSession = sqlSessionFactory.openSession(true)) {
            Map<String, Object> map = new HashMap<>();
            map.put("list", users);
            sqlSession.insert("userMapper.insertUsers", map);
            sqlSession.commit();
        }
    }
