...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<User> list = session.selectList("MyMapper.getUserByPage", map);
            int count = session.selectOne("MyMapper.count", username);
    ...
    public void deleteUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            Map<String, Object> param = new HashMap<>();
            param.put("id", id);
            session.delete("MyMapper.deleteUserById", param);
        }
    }
    ...
    public User selectUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            int countUserId = session.selectOne("MyMapper.countUserId", id);
            if (countUserId != 0) {
                Map<String, Object> param = new HashMap<>();
                param.put("id", id);
                User mapper = session.selectOne("MyMapper.selectUserById", param);
                return mapper;
            }
            return null;
        }
