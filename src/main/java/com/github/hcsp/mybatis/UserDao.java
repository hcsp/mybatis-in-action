...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        ...
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            List<User> list = session.selectList("db.mybatis.mapper.selectUser", map);
            targetUserNums = session.selectOne("db.mybatis.mapper.countPageNums", username);
            int totalPage = targetUserNums % pageSize == 0 ? targetUserNums / pageSize : targetUserNums / pageSize + 1;
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            Map map = new HashMap();
            map.put("users", users);
            session.insert("db.mybatis.mapper.insertUser", map);
        }
    }
    ...
    public void updateUser(User user) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.update("db.mybatis.mapper.updateUser", user);
        }
    }
    ...
    public void deleteUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.delete("db.mybatis.mapper.deleteUser", id);
        }
    }
    ...
    public User selectUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            User user = session.selectOne("db.mybatis.mapper.returnUser", id);
            if (user != null) {
                return user;
            }
        }
