...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = this.sqlSessionFactory.openSession()) {
            ...
            List<User> userList = session.selectList("MyMapper.getUserByPage", params);
            Integer userNum = session.selectOne("MyMapper.getTotalPage", username);
            Integer totalPage = (userNum % pageSize == 0) ? userNum / pageSize : userNum / pageSize + 1;
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = this.sqlSessionFactory.openSession(true)) {
            ...
            session.insert("MyMapper.batchInsertUsers", params);
        }
    }
