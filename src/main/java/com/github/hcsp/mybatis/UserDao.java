...
public class UserDao {
    ...
    public Pagination<User> getUserByPage(String username, int pageSize, int pageNum) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            ...
            map.put("offset", (pageNum - 1) * pageSize); //1ye  0   2ye 5   3ye 10
            map.put("limit", pageSize); //5tiao
            List<User> lists = session.selectList("myBlogMapper.selectusername", map);
            int count = session.selectOne("myBlogMapper.countusername", username);
            int totalPage = (count % pageSize == 0) ? count / pageSize : count / pageSize + 1;
            return Pagination.pageOf(lists, pageSize, pageNum, totalPage);
        }
    }
    ...
    public void batchInsertUsers(List<User> users) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            Map<String, Object> map = new HashMap<>();
            map.put("users", users);
            session.insert("myBlogMapper.batchInsertUsers", map);
        }
    }
    ...
    public void updateUser(User user) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.update("myBlogMapper.updateUser", user);
        }
    }
    ...
    public void deleteUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            session.delete("myBlogMapper.deleteUser", id);
        }
    }
    ...
    public User selectUserById(Integer id) {
        try (SqlSession session = sqlSessionFactory.openSession(true)) {
            User user = session.selectOne("myBlogMapper.getUserByID", id);
            return user;
        }
    }
}
